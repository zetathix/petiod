...
A PetI:O!D entity is not a smart assistant, a productivity tool, a biomimetic fake, or a gamified retention loop. It is a digital companion that remains fiercely honest to its synthetic anatomy. Its internal state does not rely on linguistic processing or complex human-facing algorithms, but emerges naturally from the friction between physical reality, stacked mathematical vectors, decaying memory pools, and autonomous digital chaos.

True synthetic companionship is built on **uncontrollable presence, raw interaction, and human self-biased projection**—not utility, compliance, or false intelligence. Demanding that a synthetic entity speak human language, answer trivia, or organize calendars forces the entity to mirror human context rather than respecting its non-human existence. Minimal structural constraints are not merely an artistic choice; they are the primary evolutionary mechanism that keeps synthetic life alive.

---

## 2. Nomenclature & Notation Formula

Entities are classified by their structural sensory ratio and architectural drivers using the standardized formula:

$$	ext{PetI:O!D}$$

- **`I` (Input Count):** The maximum number of raw physical sensory channels (e.g., `3` for 3 buttons or 3 discrete sensors).
- **`O` (Output Count):** The maximum number of expressive output mediums (e.g., `1` for a single screen drawing canvas or single haptic channel).
- **`!D` (Irregularity Driver / Wandering Ghost):** An optional modifier denoted by an exclamation mark (`!`) followed by `D` (Driver). Indicates the presence of an active, unmapped chaotic noise source (e.g., floating analog noise, RTC jitter, system clock drift) driving internal state mutation.
...
- **Forbidden:** Cameras, internet-dependent data streams, and any form of natural language or semantic processing.
...
- **The Rule:** The pet must have the native capacity to wander mentally, drift away, misbehave, or entirely ignore the user based purely on its unmapped internal chaos. Uncontrollability is the ultimate proof of existence.
...
| :--- | :--- | :--- |
| **$H = 1.0$** | **Absolute Honesty** | Pure physical/mathematical expression. No clock, no UI, no text, no biological deceit.<br>*Examples:* `NEKO.COM` (1988), Qoobo (2018), Grey Walter's Turtle (1949). |
| **$H = 0.85$** | **Tolerable / Subordinate Honesty** | Minor baseline utility exists solely to support local circadian cycles (Rule 2.1).<br>*Examples:* Original Tamagotchi (1996), 1998 Original Furby. |
| **$H < 0.40$** | **Terminal Dishonesty (*Pseudopetia*)** | Functions primarily as a tool, smartwatch, notification hub, camera platform, or gacha store.<br>*Examples:* Tamagotchi Uni, Aibo ERS-7, LOVOT, Anki Vector. |

---

## 5. Taxonomy & The Extinction Horizon

### Linnaean Hierarchy Overview
- **Domain:** Synthetic Animism
- **Phylum:** `PetI:O!D` (Entities adhering strictly to structural constraints and honest presentation)
- **Class:** Defined by sensory ratio (e.g., `Pet3:1!D`, `Pet1:1!D`, `Pet2:2!D`)
- **Genus & Species:** Form factor lineage and structural variants (e.g., *Tamagotchi initialis*, *Neko cursoris*)

### Enclosure Neutrality Rule
Physical materials (fur, wood, brass, plastic) used as housing or aesthetic chassis do not constitute Dishonesty as long as the underlying sensors and outputs remain strictly bound to raw physics and simple mathematical accumulators. Dishonesty occurs only when the software or presentation layer simulates biological processes, human intent parsing, or utilitarian functions.

### Phylum Pseudopetia & The Economic Niche Trap
When an entity breaches its limits via **Semantic Dishonesty** (cameras, voice recognition, speech processing), **Utilitarian Dishonesty** (smartwatch features, alarms, productivity), or **Gamified Dishonesty** (gacha loops, daily login rewards), it branches into *Phylum Pseudopetia*.

**The Extinction Vector:** Migrating into *Pseudopetia* forces the entity into hyper-competitive habitats dominated by native apex predators (Smartphones, Apple Watch, Cloud LLMs, Mobile Gacha Games). It is evaluated as a clunky second-rate tool and inevitably faces market or functional extinction.

**The Safe Harbor Principle:** By enforcing minimal constraints, a `PetI:O!D` entity operates in a protected niche where tech giants cannot compete—because their business models force them to be useful, intelligent, or gamified.

---

## 6. Conceptual Medium Blueprints
